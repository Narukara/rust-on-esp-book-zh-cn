...
- 在命令行中使用[工具链覆盖][toolchain-override]简写：`cargo +esp`。
- 将 `RUSTUP_TOOLCHAIN` 环境变量设置为 `esp`。
- 设置[目录覆盖][directory-override]：`rustup override set esp`
...
有关工具链覆盖的更多信息，请参阅 The rustup book 的[覆盖章节][overrides-rust-book]。
...
在使用 Windows 时，如果使用长路径名，你可能会在构建新项目时遇到问题。
此外，如果你正在尝试构建 `std` 应用程序，如果你的项目路径长度超过约 10 个字符，构建将失败并出现严重错误。

要解决此问题，你需要缩短你的项目名称，并将其移动到驱动器根目录，例如 `C:\myproj`。
还要注意，虽然使用 Windows `subst` 实用程序（例如 `subst r: <pathToYourProject>`）可能看起来像是在构建期间使用短路径的简单解决方案，同时仍保持项目位置不变，
...
此错误的原因是我们缺少 MSVC C++，因此我们没有满足[编译时要求]。请安装 [Visual Studio 2013（或更高版本）或 Visual C++ Build Tools 2019]。对于 Visual Studio，请确保选中"C++ 工具"和"Windows 10 SDK"选项。
如果使用 GNU ABI，请安装 [MinGW/MSYS2 工具链]。
